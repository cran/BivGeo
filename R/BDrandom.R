...
#'
#' @return \code{\link[BivGeo]{rbdbivgeo1}} and \code{\link[BivGeo]{rbdbivgeo2}} generate random deviates from the Bash-Dhar bivariate geometric distribution. The length of the result is determined by n, and is the maximum of the lengths of the numerical arguments for the other functions.
#'
...
rbdbivgeo1 <- function(n, theta)
{
  ...
  for (i in 1:n)
  {
    if(x[i] == 1 && u[i] < 1 - theta2 * theta12 * (1 - theta1)/(1 - theta1 * theta12)) {y[i] = 1}
    else
      if(x[i] == 1)
      {
        for(s in 2:n) if(u[i] >= 1 - (1 - theta1) * (theta2 * theta12)^(s - 1)/(1 - theta1 * theta12) && u[i] < 1 - (1 - theta1) * (theta2 * theta12)^s/(1 - theta1 * theta12)) {y[i] = s}
      }
    else
      for (j in 1:x[i] - 2) if(u[i] >= 1 - theta2^j && u[i] < 1 - theta2^(j + 1)) {y[i] = j + 1}
    else
      if(u[i] >= 1 - theta2^(x[i] - 1) && u[i] < 1 - (theta2^x[i]) * theta12 * (1 - theta1)/(1 - theta1 * theta12)) {y[i] = x[i]}
    else
      if(u[i] >= 1 - (theta2^x[i]) * theta12 * (1 - theta1)/(1 - theta1 * theta12) && u[i] < 1 - (1 - theta1) * (theta2^(x[i] + 1)) * theta12^2/(1 - theta1 * theta12)) {y[i] = x[i] + 1}
    else
      if(u[i] >= 1 - (1 - theta1) * (theta2^(x[i] + 1)) * theta12^2/(1 - theta1 * theta12))
      {
        for(k in 1 + x[i]:n) if(u[i] >= 1 - (1 - theta1) * theta12^(-x[i] + 1) * (theta2 * theta12)^k/(1 - theta1 * theta12) && u[i] < 1 - (1 - theta1) * theta12^(-x[i] + 1) * (theta2 * theta12)^(k+1)/(1 - theta1 * theta12)) {y[i] = k + 1}
      }
  }
